...
AddEventHandler('thermite:UseThermite', function()
    ...
    elseif currentThermiteGate ~= 0 then
        ...
        if CurrentCops >= Config.MinimumThermitePolice then
            ...
            TriggerEvent('inventory:client:requiredItems', requiredItems, false)
            --SetNuiFocus(true, true)
            exports["memorygame_2"]:thermiteminigame(8, 6, 6, 25,
            function() -- success
                ThermiteSuceed()
            end,
            function() -- failure
...
function ThermiteSuceed()
    ...
    elseif currentGate ~= 0 then
        ...
        TriggerServerEvent('nui_doorlock:server:updateState', currentGate, false, source, false, true)
        currentGate = 0
    end
end
